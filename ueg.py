...
class UEG(object):
    def __init__(self, nelec, nbasis, rs, verbose=False):
        ...
        print(f"gvecs time: {t2-t1} seconds")

        t1 = time.perf_counter()
        self.build_eri()
        t2 = time.perf_counter()
        print(f"eri time: {t2-t1} seconds")

        t1 = time.perf_counter()
        self.build_qconserv()
        t2 = time.perf_counter()
        print(f"qconserv time: {t2-t1} seconds")
        if self.verbose:
            self.print_info()
    ...
    #    self.qconserv = np.zeros([self.nbas, self.nbas, self.nbas], dtype=int)
    #    G_dict = {tuple(np.round(g, 5)): idx for idx, g in enumerate(self.rgvecs)}

    #    for i in range(self.nbas):
    #        for j in range(self.nbas):
    #            for a in range(self.nbas):
    #                G_b = self.rgvecs[i] + self.rgvecs[j] - self.rgvecs[a]
    #                g_b = tuple(np.round(G_b, 5))
    #                self.qconserv[i, a, j] = np.round(G_dict.get(g_b, -1))
    def build_qconserv(self):
